...
/*#include <gnome.h> No gnome in beos - Riegger */
#include <stdio.h>
#include <config.h>
#include <unistd.h>
#include <sys/stat.h>
#include "calendar.h"
/*#include "alarm.h" not alarms yet  - Riegger*/
#include "timeutil.h"
...
#define VERSION "michaelriegger'sversion" /* WARNING - calendar.c makees reference to 
										   * 'VERSION', defined in gnome headers? - Riegger*/

int g_file_exists(const char *fname){ /* VERY nasty solution , plz fix asap - Riegger */
	return 1; 
}
...
try_add (iCalObject *ico, CalendarAlarm *alarm, time_t start, time_t end)
{
	...
	if (alarm->trigger > calendar_day_end)
		return;
	/*alarm_add (alarm, &calendar_notify, ico); suppress this alarm stuff - Riegger*/
}
...
calendar_save (Calendar *cal, char *fname)
{
	VObject *vcal;
	FILE *fp;
	/*GtkWidget *dlg; - no gtk , fix later - Riegger */
	struct  stat s;
	int status;
...
/*		dlg = gnome_message_box_new(_("Failed to save calendar!"), 
		...
		gtk_widget_show(dlg); - no gtk - Riegger */
	}
...
calendar_init_alarms (Calendar *cal)
{
	...
	/*alarm_add (&day_change_alarm, calendar_day_change, cal); no alarms right now-Riegger*/
}
